...
func NewSigner(keyPair *KeyPair, accessTTL, refreshTTL time.Duration) *Signer {
	...
	return &Signer{keyPair: keyPair, accessTTL: accessTTL, refreshTTL: refreshTTL}
}
...
func (s *Signer) GenerateTokens(userID, email string, role userv1.UserRole) (*TokenPair, error) {
	now := time.Now()
...
func (c *Claims) ParseRole() (userv1.UserRole, error) {
	val, ok := userv1.UserRole_value[c.Role]
	if !ok {
		return 0, fmt.Errorf("unknown role: %s", c.Role)
	}
	return userv1.UserRole(val), nil
}
