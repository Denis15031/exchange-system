...
var ErrTokenExpired = errors.New("token expired or not found")
...
func (tm *TokenManager) Login(email, password string) (*userv1.LoginResponse, error) {
	...
	tm.expiresAt = time.Now().Add(DefaultAccessTTL)
	tm.mu.Unlock()
...
func (tm *TokenManager) GetAccessToken() (string, error) {
	...
	if tm.accessToken == "" || time.Now().After(tm.expiresAt) {
		return "", ErrTokenExpired // или errors.New("no valid token")
	}
...
func (tm *TokenManager) RefreshToken() (string, error) {
	...
	if tm.refreshToken == "" {
		return "", errors.New("refresh not supported in CLI mode — please login again")
	}
	...
	tm.accessToken = resp.AccessToken
	tm.expiresAt = time.Now().Add(DefaultAccessTTL)
	// RefreshToken не возвращается - оставляем старый
